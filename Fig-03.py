...
#    for SIGMA in [10, 20, 40, 80, 0]:
    for SIGMA in [10, 20, 40, 80, 160]:
        ...
        axs[idx//2, idx % 2].plot(lam[sfc:], height[sfc:], label='%2.2f' % (DX / np.mean(data['zi'])))
        axs[idx//2, idx % 2].set_ylim([0, 1.3])
        
        print(case, '%2.2f' % (lam[xi]/(np.mean(data['lt'])*1000*.1)))
...
axs[1, 0].set_xlabel('$\Lambda_1$ (m)')
axs[1, 1].set_xlabel('$\Lambda_1$ (m)')
plt.tight_layout()
plt.savefig('FIG_TB_L')