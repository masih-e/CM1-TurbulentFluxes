...
for idx, case in enumerate(['CBL24-01', 'CBL05-15', 'NBL-15', 'SBL-08']):
    ...
    X = di2(data['tke_BP'], axis=(1, 0, 0)) +data['tke_SP'] + data['tke_adv'] + (data['tke_tur'] + data['tke_pre'])  # residual term (diss.)

    axs[idx//2, idx % 2].plot(np.mean(data['tke_BP'], axis=(1, 2))[sfc:],  height1[sfc:], label='BP')
    axs[idx//2, idx % 2].plot(np.mean(data['tke_SP'], axis=(1, 2))[sfc:],  height[sfc:],  label='SP')
    axs[idx//2, idx % 2].plot(np.mean(data['tke_adv'], axis=(1, 2))[sfc:], height[sfc:],  label='ADV')
    axs[idx//2, idx % 2].plot(np.mean(data['tke_tur'], axis=(1, 2))[sfc:], height[sfc:],  label='TUR')
    axs[idx//2, idx % 2].plot(np.mean(data['tke_pre'], axis=(1, 2))[sfc:], height[sfc:],  label='PRE')
    axs[idx//2, idx % 2].plot(-np.mean(X+di2(diss[1:]), axis=(1, 2))[sfc:],   height[sfc:], 'k', label='DIS')
...
axs[0, 0].set_xlim([-.04, .03])
axs[0, 1].set_xlim([-.04, .03])
axs[1, 0].set_xlim([-.04, .03])
axs[1, 1].set_xlim([-.04, .03])

axs[1, 0].legend()
axs[0, 0].set_ylabel('Z/$z_i$')
axs[1, 0].set_ylabel('Z/$z_i$')
axs[1, 0].set_xlabel('TKE Budget')
axs[1, 1].set_xlabel('TKE Budget')
